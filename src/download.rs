...
pub(crate) fn download_with_progress(url: &str) -> StrResult<Vec<u8>> {
    let response = download(url)?;
    RemoteReader::from_response(response)
        .download()
        .map_err(|err| err.to_string().into())
}

/// Download from a URL.
fn download(url: &str) -> StrResult<ureq::Response> {
    let builder =
        ureq::AgentBuilder::new().user_agent(concat!("typst/", env!("CARGO_PKG_VERSION")));

    let agent = builder.build();
    agent.get(url).call().map_err(|err| err.to_string().into())
}
